...
export function CopdOverview({ patientId, conditions, observations, medicationRequests, tasks, encounters, medicationStatements, detectedIssues, serviceRequests, immunizations, diagnosticReports }: CopdOverviewProps) {
  ...
  return <div className="space-y-6">
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <Card className="gap-3 py-5 shadow-none"><CardHeader className="px-5"><CardTitle className="flex items-center gap-2 text-sm text-[color:var(--brand)]"><Wind className="size-4" />COPD status</CardTitle></CardHeader><CardContent className="space-y-2 px-5 text-sm"><p className="text-base font-semibold">{goldLabel}</p><p>FEV1 % predicted: {fev1Percent ?? "Insufficient data"}</p><p>FEV1: {fev1 ?? "Insufficient data"}</p><p>FEV1/FVC: {fev1Fvc ?? "Insufficient data"}</p></CardContent></Card>
      <Card className="gap-3 py-5 shadow-none"><CardHeader className="px-5"><CardTitle className="flex items-center gap-2 text-sm text-[color:var(--brand)]"><Activity className="size-4" />Respiratory status</CardTitle></CardHeader><CardContent className="space-y-2 px-5 text-sm"><p>SpO2: {spo2 ?? "Insufficient data"}</p><p>mMRC: {dyspnea ?? "Insufficient data"}</p><p>Oxygen: {oxygen ?? "Insufficient data"}</p><p>Respiratory rate: {respiratoryRate ?? "Insufficient data"}</p></CardContent></Card>
      ...
      <Card className="gap-3 py-5 shadow-none"><CardHeader className="px-5"><CardTitle className="flex items-center gap-2 text-sm text-[color:var(--brand)]"><Pill className="size-4" />Current care</CardTitle></CardHeader><CardContent className="space-y-2 px-5 text-sm"><p>Home health: {homeHealth ? "Active" : "Insufficient data"}</p><p>Pulmonary rehab: {rehab ? "Referred" : "Not documented"}</p><p>Open tasks: {openTasks.length}</p><Badge variant={openTasks.length || medicationIssue ? "warning" : "success"}>{openTasks.length || medicationIssue ? "Needs review" : "Current"}</Badge></CardContent></Card>
    </div>
    <Card className="gap-3 py-5 shadow-none"><CardHeader className="px-5"><CardTitle className="text-base text-[color:var(--brand)]">Needs attention</CardTitle></CardHeader><CardContent className="space-y-2 px-5 text-sm">{detectedIssues.length === 0 && !medicationIssue && openTasks.length === 0 ? <p>No current COPD care issues requiring review.</p> : <>{medicationIssue && <p className="text-[color:var(--warning-text)]">Medication reconciliation needs review.</p>}{openTasks.map(task => <p key={task.id}>{task.description ?? "Open clinician task"}</p>)}</>}</CardContent></Card>
    <div className="grid gap-4 xl:grid-cols-2"><ConditionsCard conditions={conditions} /><MedicationsCard medicationRequests={medicationRequests} /></div>

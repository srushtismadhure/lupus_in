...
const STATUS_MARKER: Record<SleSystemReview["status"], string> = {
  "current-activity": "bg-[var(--primary)]",
  "possible-activity": "bg-[var(--primary)]",
  "historical-involvement": "bg-[var(--sky-blue)]",
  "no-current-evidence": "bg-[var(--success)]",
  "assessment-incomplete": "bg-[var(--yellow)]",
  "monitoring-due": "bg-[var(--yellow)]",
...
function RegionButton({
  ...
  return (
    <button
      ...
      className={cn(
        "group relative flex min-h-11 w-full items-center gap-2 rounded-lg border px-3 py-2 text-left text-xs font-semibold outline-none transition-colors focus-visible:ring-[3px] focus-visible:ring-[var(--primary)]/45 focus-visible:ring-offset-2",
        selected
          ? "border-[var(--primary)] bg-[var(--info-bg)] text-[color:var(--brand)]"
          : "border-[var(--border)] bg-white text-[color:var(--foreground)] hover:border-[var(--info-border)] hover:bg-[var(--blue-panel)]",
      )}
    >
...
function Silhouette({ selected }: { selected: SleSystemId }) {
  const active = (system: SleSystemId) => selected === system ? "var(--primary)" : "var(--sky-blue)";
  return (
    <svg viewBox="0 0 180 430" className="mx-auto h-full max-h-[500px] w-full max-w-[210px]" role="img" aria-label="Front-facing body illustration. Use the labeled organ buttons to select a system.">
      <circle cx="90" cy="42" r="29" fill={selected === "constitutional" ? "var(--info-border)" : "var(--muted)"} stroke="#8796A7" strokeWidth="2" />
      <path d="M68 74 C52 84 45 108 43 150 L29 242 C27 253 35 258 42 250 L58 178 L62 270 L52 404 C51 417 66 419 70 407 L89 292 L110 407 C114 419 129 417 128 404 L118 270 L122 178 L138 250 C145 258 153 253 151 242 L137 150 C135 108 128 84 112 74 Z" fill={selected === "constitutional" ? "var(--info-bg)" : "var(--muted)"} stroke="#8796A7" strokeWidth="2" />
      ...
      <path d="M91 124 C82 112 73 125 78 139 C82 150 91 157 91 157 C91 157 103 146 104 134 C105 124 98 117 91 124" fill={active("cardiorespiratory")} stroke="var(--card)" strokeWidth="1.5" />
      ...
      {selected === "mucocutaneous" && <path d="M68 74 C52 84 45 108 43 150 L29 242 M112 74 C128 84 135 108 137 150 L151 242" fill="none" stroke="var(--primary)" strokeWidth="4" strokeLinecap="round" />}
    </svg>
  );
}
...
export function SleBodyMap({ systems, selectedSystemId, onSelect }: { systems: SleSystemReview[]; selectedSystemId: SleSystemId; onSelect: (systemId: SleSystemId) => void }) {
  ...
  return (
    <section aria-labelledby="body-map-heading" className="rounded-lg border border-[var(--border)] bg-white p-4 shadow-[0_8px_24px_rgba(31,36,48,0.05)]">
      <div className="mb-3">
        <h2 id="body-map-heading" className="text-base font-semibold text-[color:var(--foreground)]">Interactive body map</h2>
        <p className="mt-1 text-xs text-[color:var(--muted-foreground)]">Select a labeled region to review its documented evidence and assessment gaps.</p>
      </div>
      <div className="grid gap-3 lg:grid-cols-[minmax(118px,0.8fr)_minmax(150px,1fr)_minmax(118px,0.8fr)]">
        ...
        <div className="order-1 flex min-h-[340px] items-center justify-center rounded-lg bg-[var(--background)] lg:order-2 lg:min-h-[500px]"><Silhouette selected={selectedSystemId} /></div>
        <div className="order-3">{renderSide("right")}</div>
      </div>
      <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 border-t border-[var(--border)] pt-3 text-xs text-[color:var(--muted-foreground)]" aria-label="Body map status legend">
        {[
          ["bg-[var(--primary)]", "Current or possible activity"],
          ["bg-[var(--sky-blue)]", "Historical involvement"],
          ["bg-[var(--yellow)]", "Incomplete or due"],
          ["bg-[var(--success)]", "Assessment complete"],

    ...
    <aside className="flex h-screen w-60 shrink-0 flex-col justify-between bg-[#2D123F] text-[#D9CFE2]">
        ...
        <nav className="mt-2 flex flex-col gap-0.5 px-3">
          {navItems.map(item =>
            item.enabled ? (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive ? "bg-[rgba(185,220,244,0.16)] text-white" : "bg-transparent text-[#D9CFE2]",
                  )
                }
              >
              ...
              <div
                key={item.label}
                className="flex cursor-not-allowed items-center justify-between rounded-md bg-transparent px-3 py-2 text-sm text-[#D9CFE2]/50"
                title="Coming later"
              >
      ...
      <div className="border-t border-white/10 px-5 py-4">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          />
          <span className="text-[#D9CFE2]">SMART on FHIR</span>
        </div>
